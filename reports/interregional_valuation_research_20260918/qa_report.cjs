...
(async()=>{
 ...
 const screenAt=async(selector,file)=>{await page.locator(selector).evaluate(el=>el.scrollIntoView({behavior:'instant',block:'start'}));await page.screenshot({path:path.join(root,'qa',file),animations:'disabled',timeout:90000});};
 ...
 assert(integrity.interactiveCharts===9,'All nine interactive charts rendered: '+integrity.interactiveCharts);
 assert(integrity.explanatoryDiagrams===3,'All three explanatory diagrams rendered');
 ...
 const setInput=async(id,value)=>page.evaluate(([field,next])=>{const el=document.getElementById(field);el.value=next;el.dispatchEvent(new Event('input',{bubbles:true}));},[id,value]);
 await setInput('severity','4000');assert((await page.textContent('#impliedHours'))==='11.04 hours','Inverse severity relationship');
 await setInput('capB','-1');assert(await page.locator('#calcError').isVisible(),'Invalid input visibly rejected');
 await page.evaluate(()=>$('resetInputs').click());assert(!(await page.locator('#calcError').isVisible()),'Reset clears invalid state');
 const heatmaps=[];for(const metric of ['spread','energy','scarcity','scarcity_share']){await page.selectOption('#heatMetric',metric);heatmaps.push(await page.evaluate(()=>document.querySelector('#spreadHeatmap').data[0].z.flat().filter(Number.isFinite).length));}
 assert(heatmaps.every(n=>n===21),'All heatmap measures contain 21 complete-quarter cells');
 const payoffIdentity=await page.evaluate(()=>{const traces=document.querySelector('#payoffChart').data;return traces[0].y.every((v,i)=>Math.abs(v-traces[1].y[i]-traces[2].y[i])<1e-9);});
 assert(payoffIdentity,'Payoff chart closes base = energy + scarcity at every point');
 ...
 await screenAt('#visualAtlas','desktop_visual_atlas.png');
 await screenAt('#triggerMap','desktop_trigger_map.png');
 await screenAt('#modelArchitecture','desktop_architecture.png');
 await screenAt('#implementationRoadmap','desktop_roadmap.png');
 ...
 await screenAt('#visualAtlas','mobile_visual_atlas.png');
 const chartSizes=await page.evaluate(()=>Object.fromEntries(['regimeChart','scarcityMap','concentrationChart','payoffChart','spreadHeatmap','flowBridge'].map(id=>[id,document.getElementById(id).getBoundingClientRect().height])));
 assert(Object.values(chartSizes).every(h=>h>=300),'New chart collapsed on mobile '+JSON.stringify(chartSizes));
 await screenAt('#triggerMap','mobile_trigger_map.png');
 ...
 const result={passed:true,chapterCount,integrity,directionQuarterCombinations:combinations,heatmapCells:heatmaps,calc,browserErrors:errors,externalRenderingRequests:requests,mobile:overflow,checked:['six directions, seven complete quarters','four regional selectors','nine interactive charts','three explanatory diagrams','four heatmap measures','payoff identities','severity sensitivity','invalid inputs and reset','source navigation and chapter expansion','full research search','offline render','desktop and mobile screenshots','print PDF']};
 fs.writeFileSync(path.join(root,'qa','browser_checks.json'),JSON.stringify(result,null,2));
 console.log(JSON.stringify(result,null,2));await Promise.race([browser.close(),new Promise(resolve=>setTimeout(resolve,5000))]);process.exit(0);
