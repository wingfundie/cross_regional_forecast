...
For numeric accuracy report MAE, RMSE, bias, P95 absolute error and improvement against both persistence and the existing selected pipeline. For directional capacities, overstatement means `prediction − actual > threshold`, for both directions after the repository's sign normalization. Report rates above 100 and 200 MW, mean positive overstatement, and contraction-window error. Do not use MAPE around zero or signed limits.
