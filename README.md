...
- [Six-interconnector diurnal and constraint-pressure research](reports/all_interconnector_regime_research_20260921/index.html) and [complete methodology](reports/all_interconnector_regime_research_20260921/METHODOLOGY.md): observed scatterplots and regime tables for three years of flows, dispatch limits, temperature, humidity and VRE; Australian seasonal quarter blocks plus calendar-quarter sensitivity; two-year binding-constraint and DUID tightening/relief pressure analysis. Rebuild with `python scripts/build_all_ic_regime_report.py` after the six longitudinal constraint studies are complete. Its **Outage regimes** section (NOS outages, Sep 2024–Aug 2026) contains matched limit/flow effects by constraint-set family, a specific-outage lookup (asset → family → substation → area, with electrically and geographically nearby generators), spillover, event studies and booking reliability. It is built by `python scripts/build_nos_regime_section.py` from `scripts/run_nos_regime.py` stages; plan, methodology and execution log are in [`execution/nos_outage_regime_v1/`](execution/nos_outage_regime_v1/PLAN.md). The full standalone write-up of that pass (landscape, family effects, timing and weather, flow response, cross-link spillover and system footprint, constraint/DUID mechanics, event studies, connector chapters, specific-outage lookup, robustness, execution record) is [`reports/nos_outage_regime_research_20260924/index.html`](reports/nos_outage_regime_research_20260924/index.html), rebuilt with `python scripts/build_nos_outage_report.py`. Its constraint-mechanics and 12-month outlook chapters (which equations bind and set each link's limit during every outage family, and a walk-forward-backtested research outlook for booked NOS outages) come from [`execution/nos_constraint_binding_v1/`](execution/nos_constraint_binding_v1/PLAN.md) via `python scripts/run_nos_binding.py --stage …`; results in [`RESULTS_SUMMARY.md`](execution/nos_constraint_binding_v1/RESULTS_SUMMARY.md).
