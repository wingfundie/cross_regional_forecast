...
from nemic.event_atlas import settings, save, log, digest, DEFAULT, VERSION
...
def context(c,dispatch):
    ...
    w=w[['time','NSW_Sydney__temperature_2m','VIC_Melbourne__temperature_2m']]
...
def report(c):
    root=c['root'];out=ROOT/'docs/html/vni_event_atlas.html';dd=ROOT/'docs/data';dd.mkdir(exist_ok=True)
    ...
    for direction,receiver in [('upper','NSW1'),('lower','VIC1')]:
    ...
    e.to_csv(dd/'vni_event_catalogue.csv',index=False)
    a.to_csv(dd/'vni_event_attribution.csv',index=False)
    eq.to_csv(dd/'vni_event_equations.csv',index=False)
    g.to_csv(dd/'vni_event_generator_steps.csv.gz',index=False,compression={'method':'gzip','mtime':0})
    coal.to_csv(dd/'vni_event_coal_context.csv',index=False)
    pe.to_csv(dd/'vni_price_events.csv',index=False)
    pd.read_csv(root/'thresholds.csv').to_csv(dd/'vni_event_thresholds.csv',index=False)
    ...
    ranked.to_csv(dd/'vni_event_generator_contributions.csv',index=False)
    ...
    grand.to_csv(dd/'vni_event_generator_summary.csv',index=False)
    ...
    seasonal.to_csv(dd/'vni_event_seasonal.csv',index=False)
    ...
    matched.to_csv(dd/'vni_event_matched_controls.csv',index=False)
    ...
    balance.to_csv(dd/'vni_event_match_balance.csv',index=False)
    # Preserve discrepancy from the legacy detector, which reset 30-min lags each month.
    old=pd.read_csv(dd/'vni_2y_contraction_events.csv');old['time']=pd.to_datetime(old.time)
    delta=e[['time','direction']].merge(old[['time','direction']],on=['time','direction'],how='outer',indicator=True)
    delta[delta._merge.ne('both')].to_csv(dd/'vni_event_detector_changes.csv',index=False)
    notes={
        '2024-11-27':{'text':'AEMO reports heat-driven NSW demand above 11 GW, four coal units and one major gas unit offline, and transmission restrictions on local generation and imports. The published MPC episode started at 14:30, before the 14:50 and 15:00 contraction detections here. These cases therefore cannot establish that the detected contraction initiated the price spike.',
                     'url':'https://www.aemo.com.au/-/media/files/major-publications/qed/2024/qed-q4-2024.pdf'},
        '2025-11-26':{'text':'AEMO reports a cloud-driven distributed-PV decline and an Avon–Marulan line outage restricting local generation and imports from Victoria. This corroborates market context; it does not establish a specific generator as the cause.',
                     'url':'https://www.aemo.com.au/-/media/files/major-publications/qed/2025/qed-q4-2025.pdf'},
        '2025-11-25':{'text':'AEMO reports reduced distributed PV and a Lower Tumut–Canberra line outage during the NSW price spike. The published summary does not provide a complete unit-outage ledger.',
                     'url':'https://www.aemo.com.au/-/media/files/major-publications/qed/2025/qed-q4-2025.pdf'}}
    ...
    dump(root/'summary.json',summary);dump(dd/'vni_event_summary.json',summary)
    body=hero('VNI • EVENT ATLAS','When transfer limits contract','Equations, generators and prices',
        'Two years of five-minute event screening, with selected-window mechanical reconstruction and supporting market conditions.',
        ['September 2024–August 2026','VIC → NSW positive','Fixed UTC+10 NEM time','Observational study'])
    ...
    body+='<div class="metrics">'+metric('Contraction detections',f'{len(e):,}',f'{e.incident_id.nunique():,} grouped incidents')+metric('Detailed contraction cases',int(e.selected.sum()),'Plus price-only cases and matched control extracts')+metric('Exact MPC observations',exact_count,'Region × five-minute intervals; not all linked to VNI')+metric('Complete screening', '24 months','210,240 intervals per series')+'</div>'
    ...
    body+=f'<p>Exact standing-equation versions were matched for {recquality["exact_version_share"]:.1%} of retained constraint rows. {int(a.exact_step_share.eq(1).sum())} of {len(a)} selected cases have exact equation matches for every attribution step. This does not imply every LHS component is separately observed. Coal records cover all expected 22 units in every retained interval.</p>'
    ...
    body+='<ul>'+''.join(f'<li><a href="../data/vni_event_{x}.csv">{escape(x.replace("_"," "))}</a></li>' for x in downloads)+'<li><a href="../data/vni_price_events.csv">Price-first event catalogue</a></li><li><a href="../data/vni_event_generator_steps.csv.gz">Five-minute generator changes, coefficients and impacts (compressed CSV)</a></li><li><a href="../data/vni_event_coal_context.csv">Coal availability and operating-status context</a></li></ul></section>'
    ...
    def portable(value):
        ...
        return value
    (dd/'vni_event_sources.jsonl').write_text('\n'.join(json.dumps(portable(r)) for r in source_rows)+'\n',encoding='utf-8')
    body+='<p><a href="../data/vni_event_sources.jsonl">Acquisition, source hashes, filters and cleanup log</a></p>'
    payload=json.dumps(bundles,ensure_ascii=True,separators=(',',':')).replace('</','<\/')
    body+='<script>const BUNDLES='+payload+';</script>'+JAVASCRIPT
    ...
    (ROOT/'docs/VNI_TWO_YEAR_EVENT_ATLAS.md').write_text(md,encoding='utf-8')
    manifest={'method_version':VERSION,'config':str(DEFAULT.relative_to(ROOT)),'config_sha256':digest(DEFAULT),
    ...
    dump(ROOT/'docs/html/vni_event_atlas_manifest.json',manifest)
...
let tr=[trace(s,'export','Upper bound',1,'#5696b9'),trace(s.map(r=>({...r,lower:-r.import})),'lower','Lower bound',1,'#8370b4'),trace(s,'flow','Flow',1,'#282b30'),trace(s,'NSW1_RRP','NSW price',2,'#ce9a48'),trace(s,'VIC1_RRP','VIC price',2,'#268a87'),trace(s,'NSW1_mpc','MPC',2,'#aaa')];
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--config',default=str(DEFAULT));a=p.parse_args();report(settings(a.config))
