...
def build():
    ...
    for i,(result,path,f) in enumerate(zip(results,paths,all_frames)):
        ...
        body+='</article>'
    body+='</section><script>function renderVisible(){document.querySelectorAll(".lazy-plot").forEach(x=>{if(!x.closest("[hidden],details:not([open])")&&!x.dataset.rendered){let f=JSON.parse(x.querySelector("script").textContent);x.dataset.rendered="1";Plotly.newPlot(x,f.data,f.layout,{responsive:true});}});}const picker=document.getElementById("cell-picker");if(picker){picker.addEventListener("change",function(){document.querySelectorAll(".result-cell").forEach(x=>x.hidden=x.id!==this.value);renderVisible();window.dispatchEvent(new Event("resize"));});}document.addEventListener("toggle",function(){renderVisible();window.dispatchEvent(new Event("resize"));},true);renderVisible();</script>'
