...
def context(c,dispatch):
    ...
    w=w[['time','NSW_Sydney__temperature_2m','QLD_Brisbane__temperature_2m']]
...
def report(c):
    root=c['root'];out=ROOT/'docs/html/qni_event_atlas.html';dd=ROOT/'docs/data';dd.mkdir(exist_ok=True)
    ...
    e.to_csv(dd/'qni_event_catalogue.csv',index=False)
    a.to_csv(dd/'qni_event_attribution.csv',index=False)
    eq.to_csv(dd/'qni_event_equations.csv',index=False)
    g.to_csv(dd/'qni_event_generator_steps.csv.gz',index=False,compression={'method':'gzip','mtime':0})
    coal.to_csv(dd/'qni_event_coal_context.csv',index=False)
    pe.to_csv(dd/'qni_price_events.csv',index=False)
    pd.read_csv(root/'thresholds.csv').to_csv(dd/'qni_event_thresholds.csv',index=False)
    ...
    ranked.to_csv(dd/'qni_event_generator_contributions.csv',index=False)
    ...
    grand.to_csv(dd/'qni_event_generator_summary.csv',index=False)
    ...
    seasonal.to_csv(dd/'qni_event_seasonal.csv',index=False)
    ...
    matched.to_csv(dd/'qni_event_matched_controls.csv',index=False)
    ...
    balance.to_csv(dd/'qni_event_match_balance.csv',index=False)
    # Preserve discrepancy from the legacy detector, which reset 30-min lags each month.
    old=pd.read_csv(dd/'qni_2y_contraction_events.csv');old['time']=pd.to_datetime(old.time)
    delta=e[['time','direction']].merge(old[['time','direction']],on=['time','direction'],how='outer',indicator=True)
    delta[delta._merge.ne('both')].to_csv(dd/'qni_event_detector_changes.csv',index=False)
    notes={}
    ...
    dump(root/'summary.json',summary);dump(dd/'qni_event_summary.json',summary)
    body=hero('QNI • EVENT ATLAS','When transfer limits contract','Equations, generators and prices',
        'Two years of five-minute event screening, with selected-window mechanical reconstruction and supporting market conditions.',
        ['September 2024–August 2026','NSW → QLD positive','Fixed UTC+10 NEM time','Observational study'])
    ...
    body+='<div class="metrics">'+metric('Contraction detections',f'{len(e):,}',f'{e.incident_id.nunique():,} grouped incidents')+metric('Detailed contraction cases',int(e.selected.sum()),'Plus price-only cases and matched control extracts')+metric('Exact MPC observations',exact_count,'Region × five-minute intervals; not all linked to QNI')+metric('Complete screening', '24 months','210,240 intervals per series')+'</div>'
    ...
    body+='<ul>'+''.join(f'<li><a href="../data/qni_event_{x}.csv">{escape(x.replace("_"," "))}</a></li>' for x in downloads)+'<li><a href="../data/qni_price_events.csv">Price-first event catalogue</a></li><li><a href="../data/qni_event_generator_steps.csv.gz">Five-minute generator changes, coefficients and impacts (compressed CSV)</a></li><li><a href="../data/qni_event_coal_context.csv">Coal availability and operating-status context</a></li></ul></section>'
    ...
    def portable(value):
        ...
        return value
    (dd/'qni_event_sources.jsonl').write_text('\n'.join(json.dumps(portable(r)) for r in source_rows)+'\n',encoding='utf-8')
    body+='<p><a href="../data/qni_event_sources.jsonl">Acquisition, source hashes, filters and cleanup log</a></p>'
    ...
    (ROOT/'docs/QNI_TWO_YEAR_EVENT_ATLAS.md').write_text(md,encoding='utf-8')
    ...
    dump(ROOT/'docs/html/qni_event_atlas_manifest.json',manifest)
