...
class LongitudinalConstraintStudyTests(unittest.TestCase):
    ...
    def test_vsa_configuration_is_exactly_two_years_and_isolated(self):
        config = load_study("configs/constraint_vsa_2y.json")
        self.assertEqual(config["interconnector"], "V-SA")
        self.assertEqual(config["standing_start"], config["start"])
        self.assertEqual(len(months_between(config["start"], config["end"])), 24)
        self.assertEqual(config["output_dir"], "constraint_vsa_2y")
        self.assertEqual(config["raw_cache_dir"], "constraint_vsa_2y/raw")
